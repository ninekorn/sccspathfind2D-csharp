namespace AtomicTorch.CBND.CoreMod.Characters
{
        ...
        where TClientState : BaseCharacterClientState, new()
    {
        ...
        public override void ClientDeinitialize(ICharacter gameObject)
        {
            ...
            {
                var pubstate = GetPublicState(gameObject);

                if(!this.GetType().Name.ToLower().Contains("player"))
                //if (this.GetType().Name.ToLower() != "player" || this.GetType().Name.ToLower() != "playercharacter" || this.GetType().Name.ToLower() != "player character")
                {
                    sccspathfindarrays.pathfindpermobdatadeadindex[pubstate.CurrentStats.mobtypeofindex].Enqueue(pubstate.CurrentStats.indexinmainarray);

                    if (sccspathfindglobals.theplayercharacter != null)
                    {
                        adminnotify.Execute(sccspathfindglobals.theplayercharacter, "the npc id " + gameObject.Id + " is dead." + " MobIndex:" + pubstate.CurrentStats.mobtypeofindex + " Index:" + pubstate.CurrentStats.indexinmainarray);
                    }
                }
                else
                {
                    if (sccspathfindglobals.theplayercharacter != null)
                    {
                        adminnotify.Execute(sccspathfindglobals.theplayercharacter, "The player " + gameObject.Id +  " has died.");
                    }
                }
        ...
        //adminnotify.Execute(gameObject, "npc is dead3.");

        //var playercharacter = ServerCharacterAiHelper.GetClosestTargetPlayer(gameObject);
        //currentnpcglobaldata.playercharacter
        //pathfindpermobdata
        /*if (sccspathfindglobals.theplayercharacter != null)
        {
            adminnotify.Execute(sccspathfindglobals.theplayercharacter, "npc is dead3.");
        }*/

        //((ICharacter)gameObject).GetPublicState(((ICharacter)gameObject)).
        //((ICharacter)gameObject).
        //this.GetType().
        //GetPublicState(gameObject).CurrentStats.indexinmainarray
        ...
        protected override void ServerInitializeCharacter(ServerInitializeData data)
        {
            ...
            {
                adminnotify.Execute(character, "npc is dead2.");
        ...
        protected sealed override void ServerUpdate(ServerUpdateData data)
        {
            ...
            if (data.PublicState.CurrentStats.HealthCurrent <= 0)
            {
                adminnotify.Execute(character, "npc is dead0.");
            ...
            {
                adminnotify.Execute(character, "npc is dead1.");
